...
function strmv( order, uplo, trans, diag, N, A, LDA, x, strideX ) {
	...
	if ( l === null ) {
		throw new TypeError( format( '1ynFx', order ) );
	}
	u = resolveTriangle( uplo );
	if ( u === null ) {
		throw new TypeError( format( '1ynFy', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1ynGO', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1ynGP', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1ynGQ', N ) );
	}
	if ( LDA < max( 1, N ) ) {
		throw new RangeError( format( '1ynGR', N, LDA ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1ynGS', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
