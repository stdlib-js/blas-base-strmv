...
function strmv( uplo, trans, diag, N, A, strideA1, strideA2, offsetA, x, strideX, offsetX ) { // eslint-disable-line max-params, max-len
	...
	if ( u === null ) {
		throw new TypeError( format( '1ynGT', uplo ) );
	}
	t = resolveTranspose( trans );
	if ( t === null ) {
		throw new TypeError( format( '1ynGG', trans ) );
	}
	d = resolveDiagonal( diag );
	if ( d === null ) {
		throw new TypeError( format( '1ynGU', diag ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '1ynGH', N ) );
	}
	if ( strideA1 === 0 ) {
		throw new RangeError( format( '1ynGB', strideA1 ) );
	}
	if ( strideA2 === 0 ) {
		throw new RangeError( format( '1ynG2', strideA2 ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '1ynG3', strideX ) );
	}
	if ( N === 0 ) {
		return x;
	}
